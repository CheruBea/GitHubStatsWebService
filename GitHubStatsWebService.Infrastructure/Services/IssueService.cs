...
namespace GitHubStatsWebService.Infrastructure.Services
{
    public class IssueService : IIssueService
    {
        ...
        {
            HttpResponseMessage response = await _client.GetAsync($"repos/{owner}/{repoName}/issues");

            if (response.IsSuccessStatusCode)
            {
                using var responseStream = await response.Content.ReadAsStreamAsync();
                return await JsonSerializer.DeserializeAsync<List<IssueDto>>(responseStream);
            }
            else
            {
